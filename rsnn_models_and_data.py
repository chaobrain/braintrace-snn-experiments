...
class _ExpCo_Dense_Layer(bst.Module):
  """
  ...
  def __init__(
      ...
      input_ei_sep=False,
      tau_syn=10.,
      rec_init: Callable = bst.init.KaimingNormal(),
      ff_init: Callable = bst.init.KaimingNormal()
  ):
    super().__init__()
    ...
    if input_ei_sep:
      ...
      self.exe_syn = bst.nn.HalfProjAlignPostMg(
        comm=bnn.SignedWLinear(self.n_exc_in + self.n_exc_rec, n_rec, w_init=weight),
        syn=bnn.Expon.delayed(size=n_rec, tau=tau_syn),
        out=bst.nn.COBA.delayed(E=3.5),
        post=self.neu
      )

      weight = jnp.concat([4 * ff_init([self.n_inh_in, n_rec]), 4 * rec_init([self.n_inh_rec, n_rec])], axis=0)
      self.inh_syn = bst.nn.HalfProjAlignPostMg(
        comm=bnn.SignedWLinear(self.n_inh_in + self.n_inh_rec, n_rec, w_init=weight),
        syn=bnn.Expon.delayed(size=n_rec, tau=tau_syn),
        out=bst.nn.COBA.delayed(E=-0.5),
        post=self.neu
      )
    else:

      self.inp_syn = bst.nn.HalfProjAlignPostMg(
        comm=bnn.Linear(n_in, n_rec, w_init=ff_init([n_in, n_rec])),
        syn=bnn.Expon.delayed(size=n_rec, tau=tau_syn),
      ...
      self.exe_syn = bst.nn.HalfProjAlignPostMg(
        comm=bnn.SignedWLinear(self.n_exc_rec, n_rec, w_init=rec_init([self.n_exc_rec, n_rec])),
        syn=bnn.Expon.delayed(size=n_rec, tau=tau_syn),
        out=bst.nn.COBA.delayed(E=1.5),
        post=self.neu
      )

      self.inh_syn = bst.nn.HalfProjAlignPostMg(
        comm=bnn.SignedWLinear(self.n_inh_rec, n_rec, w_init=4 * rec_init([self.n_inh_rec, n_rec])),
        syn=bnn.Expon.delayed(size=n_rec, tau=tau_syn),
        out=bst.nn.COBA.delayed(E=-0.5),
        post=self.neu
      )
...
class LIF_ExpCo_Dense_Layer(_ExpCo_Dense_Layer):
  ...
  def __init__(
    ...
    neu = bnn.LIF(n_rec, tau=tau_mem, spk_fun=spk_fun, spk_reset=spk_reset, V_th=V_th)
...
class ALIF_ExpCo_Dense_Layer(_ExpCo_Dense_Layer):
  ...
  def __init__(
    ...
    neu = bnn.ALIF(n_rec, tau=tau_mem, tau_a=tau_a, beta=beta, spk_fun=spk_fun, spk_reset=spk_reset, V_th=V_th)
    super().__init__(neu=neu, n_in=n_in, n_rec=n_rec, input_ei_sep=input_ei_sep, tau_syn=tau_syn,
                     rec_init=rec_init, ff_init=ff_init)


class LIF_ExpCu_Dense_Layer(bst.Module):
  """
  ...
  def __init__(
    ...
    super().__init__()
    self.neu = bnn.LIF(n_rec, tau=tau_mem, spk_fun=spk_fun, spk_reset=spk_reset, V_th=V_th)
    self.syn = bst.nn.HalfProjAlignPostMg(
      comm=bnn.Linear(n_in + n_rec, n_rec, jnp.concat([ff_init([n_in, n_rec]), rec_init([n_rec, n_rec])], axis=0)),
      syn=bnn.Expon.delayed(size=n_rec, tau=tau_syn),
      out=bst.nn.CUBA.delayed(),
      post=self.neu
    )
...
class LIF_STDExpCu_Dense_Layer(bst.Module):
  """
  ...
  def __init__(
    ...
    super().__init__()
    self.neu = bnn.LIF(n_rec, tau=tau_mem, spk_fun=spk_fun, spk_reset=spk_reset, V_th=V_th)
    self.std = bnn.STD(n_rec, tau=tau_std, U=0.1)
    if inp_std:
      self.std_inp = bnn.STD(n_in, tau=tau_std, U=0.1)
    else:
      self.std_inp = None
...
class LIF_STPExpCu_Dense_Layer(bst.Module):
  def __init__(
      self,
    ...
    self.neu = bnn.LIF(n_rec, tau=tau_mem, spk_fun=spk_fun, spk_reset=spk_reset, V_th=V_th)
    self.stp = bnn.STP(n_rec, tau_f=tau_f, tau_d=tau_d)
    if inp_stp:
      self.stp_inp = bnn.STP(n_in, tau_f=tau_f, tau_d=tau_d)

    self.syn = bst.nn.HalfProjAlignPostMg(
      comm=bnn.Linear(n_in + n_rec, n_rec, jnp.concat([ff_init([n_in, n_rec]), rec_init([n_rec, n_rec])])),
      syn=bnn.Expon.delayed(size=n_rec, tau=tau_syn),
      out=bst.nn.CUBA.delayed(),
      post=self.neu
    )
...
class LIF_Delta_Dense_Layer(bst.Module):
  """
...
class IF_Delta_Dense_Layer(bst.Module):
  ...
  def __init__(
    ...
    super().__init__()
    self.neu = bnn.IF(n_rec, tau=tau_mem, spk_fun=spk_fun, spk_reset=spk_reset, V_th=V_th)
    w_init = jnp.concat([ff_init([n_in, n_rec]), rec_init([n_rec, n_rec])], axis=0)
    self.syn = bst.nn.HalfProjDelta(bnn.Linear(n_in + n_rec, n_rec, w_init=w_init), self.neu)
...
class ALIF_ExpCu_Dense_Layer(bst.Module):
  """
  The RTRL layer with LIF neurons and dense connected exponential current synapses.
  """
...
class ALIF_Delta_Dense_Layer(bst.Module):
  """
  The RTRL layer with LIF neurons and dense connected delta synapses.
  """
  ...
  def __init__(
    ...
    super().__init__()
    self.neu = bnn.ALIF(n_rec, tau=tau_mem, spk_fun=spk_fun, spk_reset=spk_reset, V_th=V_th, tau_a=tau_a, beta=beta)
    w_init = jnp.concat([ff_init([n_in, n_rec]), rec_init([n_rec, n_rec])], axis=0)
    self.syn = bst.nn.HalfProjDelta(bnn.Linear(n_in + n_rec, n_rec, w_init=w_init), self.neu)
...
class ALIF_STDExpCu_Dense_Layer(bst.Module):
  """
  The RTRL layer with LIF neurons and dense connected STD-based exponential current synapses.
  """
  ...
  def __init__(
    ...
    super().__init__()
    self.neu = bnn.ALIF(n_rec, tau=tau_mem, spk_fun=spk_fun, spk_reset=spk_reset, V_th=V_th, beta=beta)
    self.std = bnn.STD(n_rec, tau=tau_std, U=0.1)
    if inp_std:
      self.std_inp = bnn.STD(n_in, tau=tau_std, U=0.1)
    else:
...
class ALIF_STPExpCu_Dense_Layer(bst.Module):
  def __init__(
      self,
...
class NetWithMemSpkRegularize(bst.Module):
  """
  ...
  def spike_reg(self, target_fr: float, factor: float = 0.):
    ...
    if factor > 0.:
      ...
      for l in neurons:
        loss += (jnp.mean(l.spike) - target_fr / 1e3 * bst.environ.get_dt()) ** 2
      loss = loss * factor
    return loss
...
class ETraceDenseNet(NetWithMemSpkRegularize):
  def __init__(self, n_in, n_rec, n_out, args, spk_fun: Callable = bst.surrogate.ReluGrad()):
    super().__init__()
    ...
    for layer_idx in range(args.n_layer):
      tau_mem = bst.random.normal(args.tau_v, args.tau_v_sigma, [n_rec]) if args.tau_v_sigma > 0. else args.tau_v
      if args.model == 'lif_expco_dense':
        rec = LIF_ExpCo_Dense_Layer(
          n_rec=n_rec, n_in=n_in, tau_mem=tau_mem, tau_syn=args.tau_syn, V_th=args.V_th, input_ei_sep=layer_idx != 0,
          spk_fun=spk_fun, spk_reset=args.spk_reset,
          rec_init=bst.init.KaimingNormal(scale=args.rec_wscale),
          ff_init=bst.init.KaimingNormal(scale=args.ff_wscale)
        )
        n_in = n_rec
    ...
    # output layer
    self.out = bnn.LeakyRateReadout(in_size=n_rec,
                                    out_size=n_out,
                                    tau=args.tau_o,
...
class visualize(object):
  ...
  def plot_multilayer_lif(inp_s: jax.Array,
    ...
    times = np.arange(inp_s.shape[0]) * bst.environ.get_dt()
    ...
    for idx in np.arange(0, inp_s.shape[1], max(int(inp_s.shape[1] // num_vis), 1)):
      ...
      plt.scatter(x=event_times * bst.environ.get_dt(), y=event_ids, s=0.5)
      plt.xlim(-1, (inp_s.shape[0] + 1) * bst.environ.get_dt())
      ...
      for j in range(n_layer):
        ...
        event_times, event_ids = np.where(spk)
        plt.scatter(x=event_times * bst.environ.get_dt(), y=event_ids, s=0.5)
        plt.xlim(-1, (hid_s[j].shape[0] + 1) * bst.environ.get_dt())
      ...
      for j in range(n_layer):
        ...
        plt.xlim(-1, (hid_v[j].shape[0] + 1) * bst.environ.get_dt())
      ...
      if out_v is not None:
        ...
        plt.xlim(-1, (out_v.shape[0] + 1) * bst.environ.get_dt())
...
def _label_processing(y_local):
  ...
  return jnp.asarray(y_local, dtype=bst.environ.ditype())  # (batch,)
...
def _sequence_data_for_mlp(x_local):
  ...
  return jnp.asarray(x_local, dtype=bst.environ.dftype())
...
def _get_gesture_data(args, cache_dir):
  ...
  # transform = transforms.ToFrame(sensor_size=in_size, time_window=bst.environ.get_dt() * 1000)
...
def _get_nmnist_data(args, cache_dir=os.path.expanduser("./data"), first_saccade_only=True):
  ...
  # transform = tonic.transforms.ToFrame(sensor_size=in_shape, time_window=bst.environ.get_dt() * 1000)
