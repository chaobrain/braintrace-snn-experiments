...
parser.add_argument("--dataset", type=str, default="gesture", help="")
...
class _LIF_Delta_Dense_Layer(bst.Module):
  """
...
class _LIF_ExpCu_Dense_Layer(bst.Module):
  ...
  def __init__(
    ...
    self.syn = bst.nn.HalfProjAlignPostMg(
      comm=bnn.Linear(n_in + n_rec, n_rec, w_init),
...
class ETraceNet(bst.Module):
  ...
  def verify(self, dataloader, x_func, num_show=5, filepath=None):
    ...
    sps = [bu.math.as_numpy(out) for out in sps]
    vs = [bu.math.as_numpy(out) for out in vs]
    ...
    for i in range(min(num_show, xs.shape[1])):
      fig, gs = bts.visualize.get_figure(2, len(self.rec_layers) + 1, 3., 3.)
...
class Trainer(object):
  ...
  def predict(self, inputs, targets):
    ...
    losses, outs = bst.transform.for_loop(_step, indices, inputs)
  ...
  def memory_efficient_etrace_functions(self, input_info):
    # initialize the states
    bst.init_states(self.target, self.args.batch_size)
    # weights
    weights = self.target.states().subset(bst.ParamState)
    ...
    def reset_state(batch_size):
      bst.reset_states(self.target, batch_size)
...
def _get_shd_data(args, cache_dir=os.path.expanduser("./data"), ):
  ...
  out_shape = 20
  transform = tonic.transforms.ToFrame(sensor_size=SHD.sensor_size, n_time_bins=args.data_length)
  ...
  return bst.util.DotDict(
    {
      ...
      'out_shape': out_shape,
      'label_process': lambda x: x,
      'input_process': lambda x: np.transpose(np.reshape(x, (x.shape[0], x.shape[1], -1)), (1, 0, 2)),
    }
  )
...
def get_snn_data(args, cache_dir=os.path.expanduser("./data")):
  """
  ...
  Returns:
    ...
    - label_process: The label processing.
    - input_process: The input processing.

  """

  if args.dataset == 'gesture':
    return _get_gesture_data(args, cache_dir)
  elif args.dataset == 'shd':
    return _get_shd_data(args, cache_dir)
  elif args.dataset == 'nmnist':
    return _get_nmnist_data(args, cache_dir)
  else:
...
def network_training():
  ...
  # loading the data
  dataset = get_snn_data(global_args)
