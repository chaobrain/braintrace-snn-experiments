...
class _LIF_Delta_Dense_Layer(brainstate.nn.Module):
    ...
    def __init__(
        ...
        self.syn = brainstate.nn.DeltaProj(comm=brainscale.nn.Linear(n_in + n_rec, n_rec, w_init=w_init),
                                           post=self.neu)
...
class _ALIF_Delta_Dense_Layer(brainstate.nn.Module):
    """
    LIF neurons and dense connected delta synapses.
    """
    ...
    def __init__(
        ...
        super().__init__()
        self.neu = brainscale.nn.ALIF(n_rec, tau=tau_mem, tau_a=tau_a, beta=beta, spk_fun=spk_fun, spk_reset=spk_reset,
                                      V_th=V_th)
        rec_init: Callable = brainstate.init.KaimingNormal(rec_scale)
        ff_init: Callable = brainstate.init.KaimingNormal(ff_scale)
        w_init = jnp.concat([ff_init([n_in, n_rec]), rec_init([n_rec, n_rec])], axis=0)
...
class ETraceNet(brainstate.nn.Module):
    def __init__(
        ...
        for layer_idx in range(n_layer):
            ...
            tau_mem = u.math.maximum(tau_mem, 0.)
...
class Trainer(object):
    ...
    def etrace_train(self, inputs, targets):
        ...
        if self.args.warmup_ratio > 0:
            ...
            brainstate.compile.for_loop(lambda i, inp: model(i, inp, running_index=i), indices[:n_sim],
                                        inputs[:n_sim])
